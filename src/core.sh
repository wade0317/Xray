...
create() {
    ...
    config.json)
        ...
        is_routing='routing:{domainStrategy:"IPIfNonMatch",rules:['"$is_ban_bt"','"$is_ban_cn"','"$is_openai"',{type:"field",ip:["geoip:private"],outboundTag:"block"}]}'
        is_outbounds='outbounds:[{tag:"direct",protocol:"freedom"},{tag:"block",protocol:"blackhole"}]'
        is_server_config_json=$(jq '{'"$is_log"','"$is_dns"','"$is_routing"','"$is_outbounds"'}' <<<{})
...
change() {
    ...
    11)
        ...
        [[ $(grep -i "^Wade0317.com$" <<<$is_servername) ]] && {
            err "你干嘛～哎呦～"
        }
        add $net
        ;;
    ...
    13)
        ...
        [[ $(grep -i "^Wade0317.com$" <<<$proxy_site) ]] && {
            err "你干嘛～哎呦～"
        } || {
            load caddy.sh
            caddy_config proxy
...
del() {
    ...
    # 删除完成后：关闭不再使用的端口 + 更新订阅（change/ddel 模式跳过，由上层统一处理）
    [[ ! $is_change && ! $is_dont_auto_exit ]] && {
        load firewall.sh; fw_revoke_unused
        load subscribe.sh; gen_subscribe
    }
}
...
api() {
    ...
    [[ ! $is_api_port ]] && {
        ...
        [[ $? != 0 || ! $is_api_port || $is_api_port == "null" ]] && {
            warn "当前未启用 API, 将改用重启方式使配置生效."
            is_api_fail=1
            return
        }
    }
...
add() {
    ...
    *http)
    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m no-auto-tls 帮助(help)\e[0m: $(msg_ul https://Wade0317.com/$is_core/no-auto-tls/)\n"
    ...
    info
    # 开放防火墙端口
    [[ ! $is_gen ]] && { load firewall.sh; fw_allow_new; }
    # 更新订阅文件
    [[ ! $is_gen ]] && { load subscribe.sh; gen_subscribe; show_sub_link; }
}
...
get() {
    ...
    protocol)
        ...
        *socks*)
            net=socks
            is_protocol=socks
            [[ ! $is_socks_user ]] && is_socks_user=xray
    ...
    install-caddy)
        _green "\n安装 Caddy 实现自动配置 TLS.\n"
        msg "  架构: ${caddy_arch}"
        msg "  步骤1: 获取 Caddy 版本号..."
        load download.sh
        msg "  步骤2: 下载 Caddy 二进制..."
        download caddy
        msg "  下载完成: $is_caddy_bin ($($is_caddy_bin version 2>/dev/null | head -1))"
        msg "  步骤3: 注册 systemd 服务..."
        load systemd.sh
...
info() {
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装请查看脚本帮助: $(msg_ul https://github.com/$is_sh_repo)"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            if [[ $(type -P qrencode) ]]; then
                ...
                msg "请安装 qrencode: $(_green "$cmd install qrencode -y")"
            fi
            msg
            footer_msg
        }
...
main() {
    ...
    dd | ddel | fix | fix-all)
        ...
        *)
        ...
        # 批量删除完成后：同步防火墙 + 更新订阅
        load firewall.sh; fw_sync_after_ddel
        load subscribe.sh; gen_subscribe; show_sub_link
        ;;
    dns)
        load dns.sh
        dns_set ${@:2}
